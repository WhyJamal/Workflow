...
import { stats, quickActions } from "./config";
import { getDashboardStats } from "@/actions/job.actions";
import { TRecentJobCard } from "@/types/ui.type";
        ...
        <div className="md:col-span-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6">
          ...
          {data.recentJobs.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-zinc-400 text-sm">Нет активных заказов</p>
              <Link href={PAGES.FIND_WORK} className="mt-3 inline-block text-sm text-[#14a800] hover:underline">
                Найти работу <ChevronRight className="w-4 h-4 inline-block" />
              </Link>
            </div>
          ) : (
            <div className="space-y-4">
              {data.recentJobs.map((job: TRecentJobCard) => (
                <div key={job.id} className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                  <div>
